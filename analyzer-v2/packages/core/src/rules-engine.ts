...
export interface CustomRule {
    ...
    severity: number;
    targetPattern: string; // The literal name of the function or regex
}
...
export class CustomRulesEngine {
    ...
    public getSinks(): { functionName: string, id: string, severity: number, description: string }[] {
        return this.rules
            .filter(r => r.type === 'SINK')
            .map(r => ({ functionName: r.targetPattern, id: r.id, severity: r.severity, description: r.description }));
    }

    public getSources(): { sourcePattern: string, id: string, severity: number, description: string }[] {
        return this.rules
