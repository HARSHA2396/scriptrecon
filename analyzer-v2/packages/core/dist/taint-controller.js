...
export class TaintController {
    ...
    fileImports = new Map();
    autoFixer = new AutoRemediator();
    customRules = new CustomRulesEngine();
     ...
     */
    markTainted(variableName, sourceContext, previousFlow) {
     ...
     */
    propagateTaint(sourceName, targetVar) {
    ...
    isNodeTainted(node) {
        ...
        switch (node.type) {
            ...
            case 'MemberExpression':
                ...
                let iter = node;
                while (iter.type === 'MemberExpression')
                    iter = iter.object;
     ...
     */
    analyzeSource(code, filePath = 'virtual-file.js') {
    ...
    getVisitor(ast, sourceCode) {
        ...
        return {
                ...
                if (node.id.type === 'Identifier' && node.init) {
                    ...
                    const isReqSource = (n) => {
                        if (n.type === 'MemberExpression') {
                            if (n.object.type === 'Identifier' && n.object.name === 'req' && n.property.type === 'Identifier' && (n.property.name === 'query' || n.property.name === 'body'))
                                return true;
            ...
            CallExpression(node) {
                ...
                if (matchedSink || customSinkMatch) {
                    ...
                    node.arguments.forEach((arg) => {
                        ...
                        if (check.tainted && check.sourceVar) {
                            ...
                            if (customSinkMatch)
                                console.log(`Description: ${customSinkMatch.description}`);
                            console.log(`Taint Data Flow Path: \n  -> ${taintInfo.flow.join('\n  -> ')}`);
                            let issueRecord = {
                                type: 'security',
                ...
                if (node.callee.type === 'MemberExpression' && node.callee.property.name === 'verify') {
                    if (node.arguments.length === 3 && node.arguments[2].type === 'ObjectExpression') {
                        const algConfig = node.arguments[2].properties.find((p) => p.key.name === 'algorithms');
                        if (algConfig && algConfig.value.type === 'ArrayExpression') {
                            const hasNone = algConfig.value.elements.some((e) => e.value === 'none');
                            if (hasNone) {
            ...
            ObjectExpression(node) {
                ...
                const hasSpread = node.properties.some((prop) => prop.type === 'SpreadElement');
                if (hasSpread) {
            ...
            BinaryExpression(node) {
                ...
                if (node.operator === '===' || node.operator === '==') {
                    const checkSide = (n) => n.type === 'Literal' && typeof n.value === 'string' && (n.value.includes('SECRET') || n.value.includes('DEBUG') || n.value.includes('ADMIN'));
                    ...
                    const checkTimeVar = (n) => n.type === 'Identifier' && ['hash', 'password', 'token', 'signature'].includes(n.name.toLowerCase());
     ...
     */
    analyzeCrossFileCall(targetFile, exportName, taintedArgs) {
