...
export function analyzeCode(code) {
    ...
    const isTaintedSource = (node) => {
        if (t.isMemberExpression(node)) {
            if (t.isIdentifier(node.object) && node.object.name === 'req' && t.isIdentifier(node.property) && (node.property.name === 'query' || node.property.name === 'body'))
                return true;
            if (t.isIdentifier(node.object) && node.object.name === 'window' && t.isIdentifier(node.property) && node.property.name === 'location')
                return true;
            if (t.isIdentifier(node.property) && node.property.name === 'location')
                return true;
        }
        if (t.isCallExpression(node) && t.isMemberExpression(node.callee)) {
            if (t.isIdentifier(node.callee.object) && node.callee.object.name === 'localStorage' && t.isIdentifier(node.callee.property) && node.callee.property.name === 'getItem')
                return true;
        }
        return false;
    };
    traverse(ast, {
        enter(path) {
            ...
            if (path.isCallExpression() && t.isIdentifier(path.node.callee) && path.node.callee.name === 'eval') {
                ...
                let isTainted = false;
                if (args.length > 0 && t.isIdentifier(args[0]) && taintedVars.has(args[0].name))
                    isTainted = true;
                if (args.length > 0 && t.isTemplateLiteral(args[0])) {
                    args[0].expressions.forEach(expr => { if (t.isIdentifier(expr) && taintedVars.has(expr.name))
                        isTainted = true; });
                }
                issues.push({
                    type: 'security',
            ...
            if (path.isCallExpression() && t.isMemberExpression(path.node.callee)) {
                ...
                if (t.isIdentifier(callee.property) && callee.property.name === 'exec') {
                    ...
                    let isTainted = false;
                    if (args.length > 0) {
                        if (t.isIdentifier(args[0]) && taintedVars.has(args[0].name))
                            isTainted = true;
            ...
            if (path.isCallExpression()) {
                ...
                if (isNetworkCall) {
                    ...
                    if (args.length > 0 && t.isStringLiteral(args[0])) {
                        ...
                        issues.push({
                            type: 'endpoint',
                            message: `Endpoint/API Discovered: ${args[0].value}`,
                            line: path.node.loc?.start.line || 0
                        });
                    }
                }
                    ...
                    if (path.node.arguments.length === 3 && t.isObjectExpression(path.node.arguments[2])) {
                        ...
                        path.node.arguments[2].properties.forEach(p => {
                            if (t.isObjectProperty(p) && t.isIdentifier(p.key)) {
                                if (p.key.name === 'secure' && t.isBooleanLiteral(p.value, { value: true }))
                                    secure = true;
                                if (p.key.name === 'httpOnly' && t.isBooleanLiteral(p.value, { value: true }))
                                    httpOnly = true;
                                if (p.key.name.toLowerCase() === 'samesite')
                                    sameSite = true;
                            }
                        });
...
export { submitForAiEvaluation } from './ai-filter.js';
